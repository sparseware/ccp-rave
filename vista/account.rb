...
module Vista
  class Account < ModuleBase
    ...
    #=============================================================================
    # Generates a pin that can be used to piggy back of an authenticated session
    #=============================================================================
    def get_pin(session,conn,paths)
      conn.mime_json
      psize=0
      phash=conn.app.get_option("pinSessionsInfo");
      if phash && phash['supported']==true
        psize=phash['pinSize']
        psize.to_i if psize
        psize=phash['pinSize']
        psize.to_i if psize
    ...
    #=============================================================================
    # Allows a pin generated by another device to be used
    # to access this session
    #=============================================================================
    def allow_pin(session,conn,paths)
      broker=session.loggedin_broker(conn) #make sure we are logged in
      pin=conn['pin']
      bad_request unless pin && pin.length>2
      session[pin]=pin
    end
    ...
    def login(session,conn,paths)
      ...
      finish_login(session,conn,broker,paths)
    end
    
    def pin_login(session,conn,paths)
      pin=conn['pin']
      phash=conn.app.get_option("pinSessionsInfo");
      unauthorized unless pin && pin.length>2 && phash && phash['supported']==true
      if phash['masked']==true
        pin=pin.to_i
        mask=session["pin"];
        pin=pin-mask.to_i if mask
        #authenticate user using pin
      else
        sess=conn.app.find_session_with_value("pin",pin)
      end
      unauthorized sess && sess.has_broker?
      session['pinned_session']=sess
      finish_login(sess,conn,sess.broker,paths)
    end
    ...
    protected
    def finish_login(session,conn,broker,paths)
